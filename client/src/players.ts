export function youtube(id: string, start: number = 0, end: number = 0) {
  return `<iframe id='ytplayer' type='text/html' width='100%' height='100%' src='https://www.youtube.com/embed/${id}?version=3&loop=0&playlist=${id}&autoplay=1&rel=0&start=${start}&end=${end > 0 ? '&end=' + end : ''}&hl=en&cc_load_policy=1&cc_lang_pref=en&cc=1&hd=1&iv_load_policy=3&origin=https://www.unli.xyz' frameborder='0'
...
export function vimeo(id: string, start: number = 0) {
